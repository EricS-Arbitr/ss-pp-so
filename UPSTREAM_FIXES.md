...
Running log of issues, gaps, and suggested improvements discovered while deploying `ss-pp-so`. Candidates for PRs or discussion with the `range-development-ansible` maintainers.

> **Path note.** The range baseline playbook was `arbitr_pp_playbook.yaml` at the
> repo root until 2026-09-18; it is now `playbooks/00-baseline.yml`. Entries below
> have had that path refreshed so they still point at real code — the facts, dates
> and reasoning are untouched, and `git log --follow` has the original.
